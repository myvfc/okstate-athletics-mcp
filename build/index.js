...
const TOOLS = [
    {
        name: 'get_roster',
        description: 'Get the roster for any Oklahoma State sport including player details',
        inputSchema: {
            type: 'object',
            properties: {
    ...
    {
        name: 'get_schedule',
        description: 'Get the schedule for any Oklahoma State sport',
        inputSchema: {
            type: 'object',
            properties: {
                sport: { type: 'string', description: 'Sport name', enum: AVAILABLE_SPORTS }
    ...
    },
    {
        ...
        description: 'Get overview of all Oklahoma State sports in one call',
        inputSchema: {
            type: 'object',
            properties: {},
        },
...
app.get('/', (req, res) => {
    res.json({
        service: 'Oklahoma State Athletics MCP Server',
        status: 'running',
        tools: TOOLS.length,
        ...
        if (method === 'initialize') {
            return res.json({
                ...
                result: {
                    ...
                    capabilities: { tools: {} },
                    serverInfo: { name: 'okstate-athletics-mcp', version: '1.0.0' }
                }
            });
        }
...
app.listen(PORT, () => {
    console.log(`Oklahoma State Athletics MCP Server running on port ${PORT}`);
